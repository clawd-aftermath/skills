...
`aftermath-ts-sdk` v3.1.0 initialization, provider methods, transaction
...
The v3.1.0 SDK additions that are safe to rely on include:
...
- `Perpetuals().getGrantVaultAgentWalletTx` and `getRevokeVaultAgentWalletTx`,
  plus the corresponding `PerpetualsVault` wrapper builders;
